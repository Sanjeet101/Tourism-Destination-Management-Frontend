import React, { Component } from 'react'

class Destination extends Component{
    render(){
        return(
           <div  class="container">
               <h5 class="font-weight-light"><b>Top Destinations</b></h5>
    <h1 class="font-weight-light"><b>Choose Your Favourite Place</b></h1>
               <div class="row">
      <div class="col-md-4 mb-5">
        <div class="card h-100">
          <img class="card-img-top" src="https://pix10.agoda.net/hotelImages/287/287692/287692_14103114370023105375.jpg?s=1024x768" alt=""/>
          <div class="card-body">
            <h4 class="card-title">Nagarkot</h4>
            
            <p class="card-text">2 Days <p class="card-text">Hotel<p class="card-text">City Tour
            <p class="card-text">BBQ Dinner <p class="card-text"><h4>From Rs.7000</h4></p></p></p></p></p>
            
          </div>
          <div class="card-footer">
            <a href="/book" class="btn btn-primary">Book Now</a>
          </div>
        </div>
      </div>
      <div class="col-md-4 mb-5">
        <div class="card h-100">
          <img class="card-img-top" src="https://images.pexels.com/photos/1335393/pexels-photo-1335393.jpeg?cs=srgb&dl=pexels-pawan-kawan-1335393.jpg&fm=jpg" alt=""/>
          <div class="card-body">
            <h4 class="card-title">Kakani</h4>
            <p class="card-text">2 Days <p class="card-text">Hotel<p class="card-text">Trekking
            <p class="card-text">Dinner and Breakfast <p class="card-text"><h4>From Rs.4000</h4></p></p></p></p></p>
          </div>
          <div class="card-footer">
            <a href="/blog"  class="btn btn-primary">Book Now</a>
          </div>
        </div>
      </div>
      <div class="col-md-4 mb-5">
        <div class="card h-100">
          <img class="card-img-top" src="https://images.squarespace-cdn.com/content/v1/53ecd1bde4b0a6f9524254f8/1601028873684-C24IDL9SD75SJJ3UP0AB/ke17ZwdGBToddI8pDm48kNvT88LknE-K9M4pGNO0Iqd7gQa3H78H3Y0txjaiv_0fDoOvxcdMmMKkDsyUqMSsMWxHk725yiiHCCLfrh8O1z5QPOohDIaIeljMHgDF5CVlOqpeNLcJ80NK65_fV7S1USOFn4xF8vTWDNAUBm5ducQhX-V3oVjSmr829Rco4W2Uo49ZdOtO_QXox0_W7i2zEA/exploring-hills-nepal-blog-banner.jpg?format=2500w" alt=""/>
          <div class="card-body">
            <h4 class="card-title">Suryachaur, Nuwakot</h4>
            <p class="card-text">2 Days <p class="card-text">Hotel<p class="card-text">Trekking and Camping
            <p class="card-text">Dinner and Breakfast <p class="card-text"><h4>From Rs.4000</h4></p></p></p></p></p>
          </div>
          <div class="card-footer">
            <a href="/book" class="btn btn-primary">Book Now</a>
          </div>
        </div>
      </div>
      <div class="col-md-4 mb-5">
        <div class="card h-100">
          <img class="card-img-top" src="https://luxuryholidaytreks.com/wp-content/uploads/2016/12/DSC3580.jpg" alt=""/>
          <div class="card-body">
            <h4 class="card-title">Bhaktapur</h4>
            <p class="card-text">5 Days <p class="card-text">Hotel<p class="card-text">Travelling
            <p class="card-text"><h4>From Rs.9000</h4></p></p></p></p>
          </div>
          <div class="card-footer">
            <a href="/blog" class="btn btn-primary">Book Now</a>
          </div>
        </div>
      </div>
      <div class="col-md-4 mb-5">
        <div class="card h-100">
          <img class="card-img-top" src="https://risingnepaldaily.com/photos/1/Basantapur1.jpg" alt=""/>
          <div class="card-body">
            <h4 class="card-title">Basantapur</h4>
            <p class="card-text">2 Days <p class="card-text">Hotel<p class="card-text">Travelling
            <p class="card-text"><h4>From Rs.2000</h4></p></p></p></p>
          </div>
          <div class="card-footer">
            <a href="/book" class="btn btn-primary">Book Now</a>
          </div>
        </div>
      </div>
         <div class="col-md-4 mb-5">
        <div class="card h-100">
          <img class="card-img-top" src="https://www.wildernessexcursion.com/uploads/img/chitlang-and-markhu-hike.jpg" alt=""/>
          <div class="card-body">
            <h4 class="card-title">Chitlang</h4>
            <p class="card-text">5 Days <p class="card-text">Hotel<p class="card-text">Travelling and Camping
            <p class="card-text"><h4>From Rs.8000</h4></p></p></p></p>
          </div>
          <div class="card-footer">
            <a href="/book" class="btn btn-primary">Book Now</a>
          </div>
        </div>
      </div>
      <div class="col-md-4 mb-5">
        <div class="card h-100">
          <img class="card-img-top" src="https://www.kabiraaz.com/wp-content/uploads/2019/12/Temple-manakamana.jpg" alt=""/>
          <div class="card-body">
            <h4 class="card-title">Manakamana</h4>
            <p class="card-text">3 Days <p class="card-text">Hotel<p class="card-text">Travelling and Worship
            <p class="card-text"><h4>From Rs.10,000</h4></p></p></p></p>
          </div>
          <div class="card-footer">
            <a href="/book" class="btn btn-primary">Book Now</a>
          </div>
        </div>
      </div>
      <div class="col-md-4 mb-5">
        <div class="card h-100">
          <img class="card-img-top" src="https://nepaltraveller.com/uploads/destination/illam.jpg" alt=""/>
          <div class="card-body">
            <h4 class="card-title">Illam</h4>
            <p class="card-text">7 Days <p class="card-text">Hotel<p class="card-text">Travelling
            <p class="card-text"><h4>From Rs.30,000</h4></p></p></p></p>
          </div>
          <div class="card-footer">
            <a href="/book" class="btn btn-primary">Book Now</a>
          </div>
        </div>
      </div>
      <div class="col-md-4 mb-5">
        <div class="card h-100">
          <img class="card-img-top" src="https://i.ytimg.com/vi/BUTPCJCz5WY/maxresdefault.jpg" alt=""/>
          <div class="card-body">
            <h4 class="card-title">Taplejung</h4>
            <p class="card-text">10 Days <p class="card-text">Hotel<p class="card-text">Travelling and Worship
            <p class="card-text"><h4>From Rs.40,000</h4></p></p></p></p>
          </div>
          <div class="card-footer">
            <a href="/book" class="btn btn-primary">Book Now</a>
          </div>
        </div>
      </div>
      <div class="col-md-4 mb-5">
        <div class="card h-100">
          <img class="card-img-top" src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSxCrvuKid1-LoxQRgQ9ZKHrqQ7oCEy7z7Q-A&usqp=CAU" alt=""/>
          <div class="card-body">
            <h4 class="card-title">kavre</h4>
            <p class="card-text">3 Days <p class="card-text">Hotel<p class="card-text">Travelling and Worship
            <p class="card-text"><h4>From Rs.2000</h4></p></p></p></p>
          </div>
          <div class="card-footer">
            <a href="/book" class="btn btn-primary">Book Now</a>
          </div>
        </div>
      </div>
      <div class="col-md-4 mb-5">
        <div class="card h-100">
          <img class="card-img-top" src="https://i2.wp.com/nepalensis.com/wp-content/uploads/2020/04/Dharan.jpg?fit=1024%2C682&ssl=1" alt=""/>
          <div class="card-body">
            <h4 class="card-title">Dharan</h4>
            <p class="card-text">7 Days <p class="card-text">Hotel<p class="card-text">Travelling
            <p class="card-text"><h4>From Rs.15,000</h4></p></p></p></p>
          </div>
          <div class="card-footer">
            <a href="/book" class="btn btn-primary">Book Now</a>
          </div>
        </div>
      </div>
      <div class="col-md-4 mb-5">
        <div class="card h-100">
          <img class="card-img-top" src="https://upload.wikimedia.org/wikipedia/commons/e/e0/Phidim.jpg" alt=""/>
          <div class="card-body">
            <h4 class="card-title">Pachthar</h4>
            <p class="card-text">5 Days <p class="card-text">Hotel<p class="card-text">Travelling
            <p class="card-text"><h4>From Rs.10,000</h4></p></p></p></p>
          </div>
          <div class="card-footer">
            <a href="/book" class="btn btn-primary">Book Now</a>
          </div>
        </div>
      </div>
      <div class="col-md-4 mb-5">
        <div class="card h-100">
          <img class="card-img-top" src="https://media.tacdn.com/media/attractions-splice-spp-674x446/06/74/08/d6.jpg" alt=""/>
          <div class="card-body">
            <h4 class="card-title">Chandragiri</h4>
            <p class="card-text">2 Days <p class="card-text">Hotel<p class="card-text">Travelling and Worship
            <p class="card-text"><h4>From Rs.5000</h4></p></p></p></p>
          </div>
          <div class="card-footer">
            <a href="/book" class="btn btn-primary">Book Now</a>
          </div>
        </div>
      </div>
      <div class="col-md-4 mb-5">
        <div class="card h-100">
          <img class="card-img-top" src="https://www.altitudehimalaya.com/wp-content/uploads/2020/02/Sisneri-swimming-pool.jpg" alt=""/>
          <div class="card-body">
            <h4 class="card-title">Sisneri</h4>
            <p class="card-text">3 Days <p class="card-text">Hotel<p class="card-text">Trekking and Camping
            <p class="card-text"><h4>From Rs.8000</h4></p></p></p></p>
          </div>
          <div class="card-footer">
            <a href="/book" class="btn btn-primary">Book Now</a>
          </div>
        </div>
      </div>
      <div class="col-md-4 mb-5">
        <div class="card h-100">
          <img class="card-img-top" src="https://subhayatra.com/wp-content/uploads/2017/12/Patan.jpg" alt=""/>
          <div class="card-body">
            <h4 class="card-title">Patan</h4>
            <p class="card-text">2 Days <p class="card-text">Hotel<p class="card-text">Travelling and Worship
            <p class="card-text"><h4>From Rs.4000</h4></p></p></p></p>
          </div>
          <div class="card-footer">
            <a href="/book" class="btn btn-primary">Book Now</a>
          </div>
        </div>
      </div>
    </div>
    </div>
        )
    }
}
export default Destination;